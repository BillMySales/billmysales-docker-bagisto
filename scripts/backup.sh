#!/bin/bash
# Backups of the Bagisto database and its storage (uploads: product, category
# and theme images, imports, invoices). APP_KEY (in .env) isn't in them: keep it.
#
...
backup() {
    ...
    # Without Laravel's caches (full page cache, compiled views): regenerated.
    tar -C "${DATA_DIR}" --exclude='storage/framework/cache/*' --exclude='storage/framework/views/*' \
        -czf "${tmp}-files.tar.gz" storage
...
restore() {
    ...
    echo "==> Restored ${ts} (docker compose up -d: setup clears Bagisto's caches)"
}
